...
public class GrassyPeatBlock extends AbstractSpreadingBlock implements BonemealableBlock {
	public static final MapCodec<GrassyPeatBlock> CODEC = simpleCodec(GrassyPeatBlock::new);

	public GrassyPeatBlock(Properties properties) {
		super(properties, Smptg.key(Registries.BLOCK, "peat"));
	}

	@Override
	public MapCodec<GrassyPeatBlock> codec() {
		return CODEC;
	}

	@Override
	protected BlockState getSpreadState(ServerLevel level, BlockPos pos) {
		return GrowthSpreadingUtil.getGrassSpreadState(level, pos);
	}
